...
{phang}
{opt nfolds(#)} specifies the number of folds for cross-validation.
Default is 10.

{phang}
{opt nlambda(#)} specifies the number of lambda values in the regularization
path for cross-validation and information criteria methods. Default is 100.

{phang}
{opt lminratio(#)} specifies the ratio of lambda_min to lambda_max in the
regularization path. Default is 0.01.

{phang}
{opt cluster(varname)} specifies the cluster variable for computing
cluster-robust penalty weights in the plugin method.


{dlgtab:Estimation}

{phang}
{opt post} requests post-lasso estimation, which refits the model without
penalization using only the variables selected by the penalized estimation.
This removes the shrinkage bias from the coefficient estimates.

{phang}
{opt nostandardize} prevents standardization of regressors before penalization.
By default, regressors are standardized to have mean zero and unit variance
during estimation.

{phang}
{opt tolerance(#)} specifies the convergence tolerance for the IRLS algorithm.
Default is 1e-8.
...
{pstd}
K-fold cross-validation partitions the data into K folds, fits the model on K-1
folds, and evaluates prediction performance on the held-out fold. This is
repeated for each fold and for each lambda in a grid. The lambda that minimizes
mean out-of-sample deviance is selected.


{dlgtab:Information Criteria}

{pstd}
BIC, AIC, and EBIC select lambda by minimizing an information criterion that
balances model fit against model complexity. EBIC (Extended BIC) includes an
additional penalty for the number of potential regressors, making it more
conservative in high-dimensional settings.
